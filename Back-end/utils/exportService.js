...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  body { font-family: 'Georgia', serif; font-size: 10pt; color: #1a1a1a; background: #fff; padding: 40px 50px; max-width: 800px; margin: 0 auto; }
  .name { font-size: 26pt; font-weight: bold; color: #1a1a1a; letter-spacing: -0.5px; }
  ...
  .divider { border: none; border-top: 2px solid #1a1a1a; margin: 16px 0 12px; }
  .section-title { font-size: 9pt; font-weight: bold; text-transform: uppercase; letter-spacing: 1.5px; color: #1a1a1a; margin-bottom: 10px; }
  ...
  .job-title { font-size: 10pt; font-weight: bold; color: #1a1a1a; }
  ...
  .cert-item { font-size: 9pt; color: #374151; margin-bottom: 4px; }
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .header { background: linear-gradient(135deg, #1e40af 0%, #3b82f6 100%); color: white; padding: 30px 40px; }
  ...
  .sidebar-title { font-size: 7.5pt; font-weight: 800; text-transform: uppercase; letter-spacing: 1.5px; color: #1e40af; margin-bottom: 6px; }
  ...
  .skill-tag { background: white; border: 1px solid #bfdbfe; border-radius: 3px; padding: 2px 7px; font-size: 7.5pt; color: #1e40af; margin-bottom: 3px; display: inline-block; margin-right: 2px; }
  .cert-mini { font-size: 7.5pt; color: #475569; margin-bottom: 4px; line-height: 1.4; }
  .section { margin-bottom: 18px; }
  .section-title { font-size: 9pt; font-weight: 800; text-transform: uppercase; letter-spacing: 1.2px; color: #1e40af; border-bottom: 2px solid #bfdbfe; padding-bottom: 4px; margin-bottom: 10px; }
  ...
  .date-badge { background: #dbeafe; color: #1e40af; padding: 1px 7px; border-radius: 10px; font-size: 7.5pt; font-weight: 600; white-space: nowrap; margin-left: 8px; }
  ...
  const exp = (data.experience || []).map(e => `
    <div style="margin-bottom:14px;">
      <div style="display:flex;justify-content:space-between;align-items:baseline;">
        <div style="font-weight:600;color:#111;">${e.role || ''}</div>
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .name { font-size: 28pt; font-weight: 300; letter-spacing: -1px; color: #111; }
  ...
  .section-title { font-size: 7.5pt; font-weight: 700; text-transform: uppercase; letter-spacing: 2px; color: #9ca3af; margin-bottom: 10px; }
  ...
  .edu-degree { font-size: 9.5pt; font-weight: 600; color: #111; }
...
<body>
  ...
  <div class="section">
    ...
    ${data.projects.map(pr => `
      <div style="margin-bottom:10px;">
        <div style="font-weight:600;color:#111;font-size:9.5pt;">${pr.name || ''}${pr.url ? ` <a href="${pr.url}" style="font-size:8pt;color:#6b7280;">[↗]</a>` : ''}</div>
  ...
  <div class="section">
    <div class="section-title">Certifications</div>
    <hr class="thin-line"/>
    ${data.certifications.map(c => `<div style="font-size:8.5pt;color:#4b5563;margin-bottom:4px;">${c.name}${c.issuer ? ` — ${c.issuer}` : ''}${c.date ? ` (${c.date})` : ''}</div>`).join('')}
  </div>` : ''}
</body>
</html>`;
};
...
const buildCreativeHTML = (data) => {
  ...
  const projects = (data.projects || []).map(pr => `
    <div class="card-item">
      <div class="card-title">${pr.name || ''}${pr.url ? ` <a href="${pr.url}" style="font-size:7.5pt;color:#059669;">[↗]</a>` : ''}</div>
      ${pr.description ? `<p class="desc">${pr.description}</p>` : ''}
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .left-bar { width: 210px; min-width: 210px; background: linear-gradient(160deg, #065f46 0%, #047857 60%, #059669 100%); color: white; padding: 30px 18px; display: flex; flex-direction: column; gap: 0; }
  ...
  .side-title { font-size: 6.5pt; font-weight: 800; text-transform: uppercase; letter-spacing: 2px; color: rgba(255,255,255,0.55); margin-bottom: 7px; }
  ...
  .skill-bubble { background: rgba(255,255,255,0.15); border: 1px solid rgba(255,255,255,0.25); border-radius: 20px; padding: 2px 8px; font-size: 7pt; color: rgba(255,255,255,0.9); }
  .cert-item { font-size: 7.5pt; color: rgba(255,255,255,0.8); margin-bottom: 5px; line-height: 1.4; }
  .main { flex: 1; padding: 28px 28px; overflow: hidden; }
  .section { margin-bottom: 20px; }
  .section-header { display: flex; align-items: center; gap: 8px; margin-bottom: 12px; }
  .section-dot { width: 10px; height: 10px; border-radius: 50%; background: #059669; flex-shrink: 0; }
  .section-title { font-size: 9pt; font-weight: 800; text-transform: uppercase; letter-spacing: 1.5px; color: #065f46; }
  .section-line { flex: 1; height: 1px; background: #d1fae5; }
  .summary { font-size: 9pt; color: #374151; line-height: 1.65; }
  .card-item { margin-bottom: 12px; padding: 10px 12px; border-radius: 6px; background: #f0fdf4; border-left: 3px solid #059669; }
  .card-row { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 3px; }
  .card-title { font-size: 9.5pt; font-weight: 700; color: #1a1a1a; }
  .card-sub { font-size: 8pt; color: #6b7280; }
  .date-chip { background: #059669; color: white; padding: 1px 8px; border-radius: 20px; font-size: 7pt; font-weight: 700; white-space: nowrap; margin-left: 8px; }
  ...
  .tech-chip { background: #d1fae5; color: #065f46; padding: 1px 7px; border-radius: 3px; font-size: 7pt; font-weight: 600; }
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  body { font-family: 'Georgia', 'Times New Roman', serif; font-size: 10pt; color: #1a1a1a; background: #fff; }
  .exec-header { background: #0f2d52; padding: 36px 50px; position: relative; }
  .exec-header::after { content: ''; position: absolute; bottom: 0; left: 0; right: 0; height: 4px; background: linear-gradient(90deg, #c9a84c, #e8c97e, #c9a84c); }
  .exec-name { font-size: 28pt; font-weight: 400; color: #ffffff; letter-spacing: 1px; font-family: 'Georgia', serif; }
  .exec-title { font-size: 11pt; color: #c9a84c; margin-top: 6px; letter-spacing: 2px; text-transform: uppercase; font-family: 'Helvetica Neue', sans-serif; font-weight: 300; }
  ...
  .exec-contact a { color: #c9a84c; text-decoration: none; }
  .body { padding: 32px 50px; }
  .two-col { display: grid; grid-template-columns: 1fr 220px; gap: 36px; }
  .section { margin-bottom: 24px; }
  .section-title { font-size: 7.5pt; font-weight: 400; text-transform: uppercase; letter-spacing: 3px; color: #0f2d52; padding-bottom: 6px; border-bottom: 1px solid #c9a84c; margin-bottom: 14px; font-family: 'Helvetica Neue', sans-serif; }
  ...
  .exp-left {}
  .exp-role { font-size: 10.5pt; font-weight: bold; color: #0f2d52; }
  .exp-company { font-size: 9pt; color: #6b7280; font-style: italic; margin-top: 1px; }
  .exp-dates { font-size: 8.5pt; color: #c9a84c; white-space: nowrap; margin-left: 10px; font-family: 'Helvetica Neue', sans-serif; font-weight: 600; }
  ...
  .side-section-title { font-size: 7pt; font-weight: 400; text-transform: uppercase; letter-spacing: 2.5px; color: #0f2d52; padding-bottom: 5px; border-bottom: 1px solid #e5e7eb; margin-bottom: 10px; font-family: 'Helvetica Neue', sans-serif; }
  .side-skill { font-size: 8.5pt; color: #374151; padding: 3px 0; border-bottom: 1px dotted #f3f4f6; line-height: 1.4; }
  .side-skill:last-child { border-bottom: none; }
  .cert-block { font-size: 8.5pt; color: #374151; margin-bottom: 6px; line-height: 1.4; }
  .proj-block { margin-bottom: 10px; }
  .proj-name { font-size: 9.5pt; font-weight: bold; color: #0f2d52; }
  .proj-tech { font-size: 7.5pt; color: #c9a84c; margin-top: 2px; letter-spacing: 0.3px; }
    ...
    <div class="two-col">
      ...
      <div class="side-box">
        ...
        <div class="side-section">
          ...
          ${data.certifications.map(c => `<div class="cert-block">${c.name}${c.issuer ? `<br><em style="font-size:8pt;color:#9ca3af">${c.issuer}</em>` : ''}${c.date ? ` <span style="font-size:7.5pt;color:#c9a84c">(${c.date})</span>` : ''}</div>`).join('')}
        </div>` : ''}
      </div>
    </div>
  </div>
...
const exportAsDocx = async (resumeData, templateId = 'classic') => {
  ...
  const accentColor = templateId === 'modern' ? '1e40af' : templateId === 'minimal' ? '111111' : templateId === 'creative' ? '065f46' : templateId === 'executive' ? '0f2d52' : '000000';
...
const exportCoverLetterAsPdf = async (text) => {
  ...
  <head>
    <meta charset="UTF-8">
    <style>
      body { font-family: 'Georgia', serif; font-size: 11pt; color: #1a1a1a; padding: 50px 60px; line-height: 1.6; max-width: 800px; margin: 0 auto; }
