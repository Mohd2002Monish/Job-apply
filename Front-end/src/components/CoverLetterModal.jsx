...
const CoverLetterModal = ({ job, onClose, onSaved }) => {
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            className="w-full accent-indigo-600 h-1 bg-slate-200 dark:bg-zinc-700 rounded-lg appearance-none cursor-pointer"
          />
        </div>
        ...
        <div>
          ...
          <div className="relative">
            <select
              ...
              className="w-full pl-3 pr-10 py-2 text-xs bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/15 focus:border-indigo-500 dark:focus:ring-indigo-500/10 appearance-none text-slate-800 dark:text-slate-200 font-medium"
              ...
              return (
                <button
                  ...
                  className={`p-2.5 rounded-xl border text-left transition-all duration-200 relative group cursor-pointer ${
                    isSelected
                      ? 'border-indigo-600 dark:border-indigo-500 bg-indigo-50/50 dark:bg-indigo-500/10 shadow-sm ring-1 ring-indigo-600/30'
                      : 'border-slate-200/80 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 hover:border-slate-300 dark:hover:border-zinc-700 hover:bg-slate-50/50 dark:hover:bg-zinc-800/35'
                  }`}
                >
                  <div className="flex items-start gap-2">
                    <span className="text-base shrink-0 mt-0.5">{t.emoji}</span>
                    <div className="overflow-hidden">
                      <p className={`text-[11px] font-bold truncate transition-colors ${isSelected ? 'text-indigo-700 dark:text-indigo-300' : 'text-slate-700 dark:text-slate-200'}`}>
                        {t.id}
                      </p>
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      {/* Modal Card */}
      <div className="relative w-full max-w-5xl bg-white/98 dark:bg-zinc-900/98 backdrop-blur-md border border-slate-200 dark:border-zinc-800 rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.15)] flex flex-col max-h-[85vh] overflow-hidden transform transition-all duration-300 scale-100 animate-fade-in">
        ...
        <div className="flex items-center justify-between px-6 py-4.5 border-b border-slate-100 dark:border-zinc-800 shrink-0 bg-gradient-to-r from-indigo-50/50 via-slate-50/50 to-indigo-50/50 dark:from-indigo-950/10 dark:via-zinc-900/10 dark:to-indigo-950/10">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-zinc-400 mt-1">
              Position: <span className="font-semibold text-slate-800 dark:text-zinc-200">{job.job}</span> • Recruiter: <span className="font-mono text-slate-600 dark:text-slate-350">{job.email}</span>
            </p>
          </div>
          <button
            onClick={onClose}
              ...
              <div className="lg:col-span-7 flex flex-col min-h-0 gap-3">
                <div className="flex items-center justify-between flex-wrap gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    {uploadedFileName && (
                      <div className="inline-flex items-center gap-1.5 text-[10px] font-bold text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 pl-2 pr-1 py-0.5 rounded-full">
                        <span>📎 {uploadedFileName.length > 20 ? uploadedFileName.slice(0, 18) + '…' : uploadedFileName}</span>
                        <button
                          type="button"
                          onClick={handleClearFile}
                          className="p-0.5 text-emerald-600 hover:text-rose-500 dark:text-emerald-400 dark:hover:text-rose-400 transition-colors"
                          title="Clear file"
                        >
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 rounded-lg transition-all cursor-pointer"
                    >
                      {uploadingFile ? (
              ...
              <div className="lg:col-span-5 flex flex-col min-h-0 gap-4">
                ...
                <div className="shrink-0 pt-3 border-t border-slate-100 dark:border-zinc-800/80">
                  <button
                    ...
                    className="w-full py-3.5 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 text-white font-bold text-xs shadow-md shadow-indigo-500/15 hover:shadow-lg hover:shadow-indigo-500/25 active:scale-[0.99] transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 cursor-pointer"
                  >
                    {generating ? (
                      <>
