...
const MicIcon = () => <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" y1="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/></svg>;
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div className="space-y-1">
              ...
              <Select
                ...
                ].find(o => o.value === form.status)}
                onChange={(opt) => setForm(f => ({ ...f, status: opt ? opt.value : 'saved' }))}
                options={[
                  { value: 'saved', label: 'Saved' },
                  { value: 'applied', label: 'Applied' },
                  { value: 'opened', label: 'Opened' },
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
        ...
        <div className="flex items-center gap-2.5 w-full sm:w-auto">
          ...
          <Select
            ...
            onChange={(opt) => setStatus(opt ? opt.value : 'all')}
            options={[
              { value: 'all', label: 'All Statuses' },
              { value: 'saved', label: 'Saved' },
              { value: 'applied', label: 'Applied' },
        ...
        <div className="px-4 py-3 border-t border-slate-100 dark:border-zinc-800 text-xs text-slate-500 dark:text-zinc-400 flex flex-col sm:flex-row items-center justify-between gap-3 bg-slate-50/50 dark:bg-zinc-900/50">
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <div className="w-24">
                <Select
                  value={[5, 10, 20, 50].map(size => ({ value: size, label: size.toString() })).find(o => o.value === limit)}
                  onChange={(opt) => { if (opt) { setLimit(opt.value); setPage(1); } }}
                  options={[5, 10, 20, 50].map(size => ({ value: size, label: size.toString() }))}
                  styles={getReactSelectStyles()}
