...
import { ChevronUpIcon, ChevronDownIcon, SearchIcon, DeleteIcon, EditIcon, PhoneIcon } from "@chakra-ui/icons";
...
const ContactsTable = () => {
    ...
    const [deleteContactId, setDeleteContactId] = React.useState(null);
    ...
    const getFilteredContacts = () => {
        return getSortedContacts().filter((contact) =>
            Object.values(contact).some((value) => String(value).toLowerCase().includes(filterText.toLowerCase()))
        );
    };
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            await axios.delete(`${apiUrl}/contacts/${deleteContactId}`);
            toast({
    ...
    const handleToggleActive = async (contact) => {
        try {
            const { data } = await axios.put(`${apiUrl}/contacts/${contact._id}`, { isActive: !contact.isActive });
            setContacts((prev) => prev.map((c) => (c._id === data._id ? { ...c, ...data } : c)));
            toast({
                title: `Contact ${data.isActive ? "activated" : "deactivated"}`,
                status: "success",
                duration: 2000,
                isClosable: true,
            });
    ...
    const handleUpdateSubmit = async () => {
        if (!editForm || !editForm._id || !editForm.name?.trim() || !editForm.contact?.trim()) {
            toast({
        ...
        try {
            const { data } = await axios.put(`${apiUrl}/contacts/${editForm._id}`, {
                name: editForm.name,
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setNewContact({ name: "", contact: "", notes: "", isActive: true });
            fetchContacts();
            toast({
    ...
    return (
        ...
        >
                    ...
                    <Table variant="simple" size="sm">
                        <Thead bg={hoverBgColor}>
                            <Tr borderBottomWidth="1px" borderColor={borderColor}>
                                ...
                                <Th cursor="pointer" onClick={() => requestSort("contact")}>
                                    <HStack spacing={2}>
                                ...
                                <Th cursor="pointer" onClick={() => requestSort("notes")}>
                                    <HStack spacing={2}>
                                        <Text>Notes</Text>
                                        ...
                                        <HStack spacing={2} justifyContent="center">
                                            ...
                                            >
                                                <Box
                                                    sx={{
                                                        ".chakra-switch__track": {
                                                            bg: contact.isActive !== false ? "green.500" : "red.500",
                                                        },
                                                ...
                                                >
                                                    <Switch
                                                        size="sm"
                                                        isChecked={contact.isActive !== false}
            ...
            {/* Update Modal */}
            <Modal isOpen={isUpdateModalOpen} onClose={onUpdateModalClose} size="lg">
                <ModalOverlay />
                <ModalContent bg={bgColor}>
                    ...
                    <ModalFooter>
                        <HStack spacing={2}>
                            ...
                            <Button colorScheme="blue" isLoading={updateLoading} onClick={handleUpdateSubmit}>
                                Save
                            </Button>
                        </HStack>
                    </ModalFooter>
                ...
                <AlertDialogOverlay>
                    <AlertDialogContent>
                        ...
                        <AlertDialogFooter>
                            ...
                            <Button colorScheme="red" onClick={handleDeleteConfirm} ml={3} isLoading={deleteLoading}>
                                Delete
                            </Button>
                        </AlertDialogFooter>
                    </AlertDialogContent>
